...
public class ListPanel extends JPanel {
	...
	public void updateMenuPanel() {
		...
		try {
			...
			if (hasResultSet == true) {
				...
				while (result.next()) {
					String s = String.format("Test %d: %s  --  %s  --  %s  --  %s finish", result.getInt("TestID"),
							result.getString("Name"), result.getString("Score"), result.getString("Time"),
							result.getString("Date"));
					testButtons.add(new JButton(s));
					testButtons.get(i).setFont(new Font("·L³n¥¿¶ÂÅé", Font.BOLD, 16));
	...
	public void createDeleteButton() {
		deleteBtn = new JButton("<html><u>Delete Note</u></html>");
